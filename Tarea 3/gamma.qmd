...
alpha_tcl <- 2.5
lambda_tcl <- 1.8
media_orig_gamma <- alpha_tcl / lambda_tcl
varianza_orig_gamma <- alpha_tcl / (lambda_tcl^2)
tamanos_muestra_tcl <- c(5, 10, 20, 30, 50, 100)
num_muestras <- 1000
set.seed(654) 

lista_medias_gamma <- list()

for (n in tamanos_muestra_tcl) {

  medias <- replicate(num_muestras, mean(rgamma(n, shape = alpha_tcl, rate = lambda_tcl)))
  lista_medias_gamma[[paste("n =", n)]] <- medias
}

data_tcl_gamma <- data.frame()
for (nombre_n in names(lista_medias_gamma)) {
  temp_df <- data.frame(media_muestral = lista_medias_gamma[[nombre_n]], n_label = nombre_n)
  data_tcl_gamma <- rbind(data_tcl_gamma, temp_df)
}
data_tcl_gamma$n_label <- factor(data_tcl_gamma$n_label, levels = paste("n =", tamanos_muestra_tcl))
tabla_tcl_stats_gamma <- data.frame()
for (nombre_n in names(lista_medias_gamma)) {
    n_val <- as.numeric(gsub("n = ", "", nombre_n))
    medias_actuales <- lista_medias_gamma[[nombre_n]]
    
    media_obs <- mean(medias_actuales)
    var_obs <- var(medias_actuales)
    
    media_teo_barraX <- media_orig_gamma
    var_teo_barraX <- varianza_orig_gamma / n_val
    
    temp_stats <- data.frame(n = n_val, Media_Obs = media_obs, Media_Teo = media_teo_barraX,
                             Var_Obs = var_obs, Var_Teo = var_teo_barraX)
    tabla_tcl_stats_gamma <- rbind(tabla_tcl_stats_gamma, temp_stats)
}

cat("Tabla de Estadísticas de las Medias Muestrales (Gamma vs TCL):\n")
tabla_kable_tcl <- kable(tabla_tcl_stats_gamma, digits = 5, 
                         caption = "Estadísticas Observadas vs. Teóricas (TCL)")
tabla_formateada_tcl <- kable_styling(tabla_kable_tcl, bootstrap_options = "striped")
print(tabla_formateada_tcl)

grafico_tcl_gamma <- ggplot(data_tcl_gamma, aes(x = media_muestral)) +
  geom_histogram(aes(y = after_stat(density)), bins = 25, fill = "lightblue", alpha = 0.7, color = "white") +

  stat_function(
    fun = dnorm,

    args = list(mean = media_orig_gamma, sd = sqrt(varianza_orig_gamma / as.numeric(gsub("n = ", "", data_tcl_gamma$n_label)))),
    color = "darkgreen", 
    linewidth = 1.1,
    linetype = "dashed"
  ) +
  ...
  labs(
    title = paste("Verificación del TCL con Distribución Gamma(", alpha_tcl, ",", lambda_tcl, ")"),
    subtitle = "Distribución de las medias muestrales vs. Normal Teórica (línea roja)",
    x = "Media Muestral",
    y = "Densidad"
  ) +
  theme_minimal()

print(grafico_tcl_gamma)
...
```{r}
#| label: solucion-suma-exponenciales
#| code-fold: true
#| warning: false
#| message: false
...
for (n in valores_n) {
  ...
  lista_sumas[[paste("n =", n)]] <- sumas
}

data_sumas <- data.frame()
for (nombre_n in names(lista_sumas)) {
  temp_df <- data.frame(suma_exp = lista_sumas[[nombre_n]], n_label = nombre_n)
  data_sumas <- rbind(data_sumas, temp_df)
}

data_sumas$n_label <- factor(data_sumas$n_label, levels = paste("n =", valores_n))

grafico_sumas <- ggplot(data_sumas, aes(x = suma_exp)) +
  geom_histogram(aes(y = after_stat(density)), bins = 30, fill = "green", alpha = 0.7, color = "white") +

  stat_function(
    fun = dgamma,
    args = list(shape = as.numeric(gsub("n = ", "", data_sumas$n_label)), rate = lambda_exp),
    color = "purple", 
    linewidth = 1.2
  ) +
  facet_wrap(~n_label, scales = "free") + 
  labs(
    title = "Suma de n Variables Exponenciales vs. Distribución Gamma(n, λ)",
    subtitle = paste("λ =", lambda_exp, ". Curva amarilla: PDF Gamma teórica"),
    x = "Suma de n Exponenciales",
    y = "Densidad"
  ) +
  theme_minimal()
...
cat("\nComparación de Medias y Varianzas (Suma Exp vs Gamma Teórica):\n")
cat("============================================================\n")
tabla_sumas_stats <- data.frame()
for (nombre_n in names(lista_sumas)) {
    n_val <- as.numeric(gsub("n = ", "", nombre_n))
    sumas_actuales <- lista_sumas[[nombre_n]]
    
    media_obs <- mean(sumas_actuales)
    var_obs <- var(sumas_actuales)
    
    # Teóricos para Gamma(n, lambda)
    media_teo <- n_val / lambda_exp
    var_teo <- n_val / (lambda_exp^2)
    
    temp_stats <- data.frame(n = n_val, Media_Obs = media_obs, Media_Teo = media_teo,
                             Var_Obs = var_obs, Var_Teo = var_teo)
    tabla_sumas_stats <- rbind(tabla_sumas_stats, temp_stats)
}
tabla_kable_sum <- kable(tabla_sumas_stats, digits = 4, 
                         caption = "Estadísticas Observadas vs. Teóricas (Suma Exponenciales)")
tabla_formateada_sum <- kable_styling(tabla_kable_sum, bootstrap_options = "striped")
print(tabla_formateada_sum)
```
