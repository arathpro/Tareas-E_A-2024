...
mediciones <- rnorm(n_sim, mean = mu_orig, sd = sigma_orig)

fuera_sim <- sum(mediciones < lim_inf | mediciones > lim_sup)
porc_fuera_sim <- (fuera_sim / n_sim) * 100

prob_fuera_teo <- pnorm(lim_inf, mu_orig, sigma_orig) + (1 - pnorm(lim_sup, mu_orig, sigma_orig))
porc_fuera_teo <- prob_fuera_teo * 100

cat("Resultados Control de Temperatura (Proceso Centrado):\n")
cat("===================================================\n")
cat("2. Porcentaje fuera de especificación:\n")
cat("   - Simulado (n=", n_sim, "): ", round(porc_fuera_sim, 2), "%\n", sep="")
cat("   - Teórico: ", round(porc_fuera_teo, 2), "%\n\n")


mu_nuevo <- 355
prob_fuera_nuevo_teo <- pnorm(lim_inf, mu_nuevo, sigma_orig) + (1 - pnorm(lim_sup, mu_nuevo, sigma_orig))
porc_fuera_nuevo_teo <- prob_fuera_nuevo_teo * 100

cat("3. Proceso Descentrado (μ = 355):\n")
cat("   - Nuevo porcentaje teórico fuera de especificación: ", round(porc_fuera_nuevo_teo, 2), "%\n\n")

z_critico <- qnorm(0.9995)
sigma_nuevo_req <- (lim_sup - mu_orig) / z_critico

cat("4. Sigma Requerido para 99.9% de Calidad:\n")
cat("   - Z crítico para 0.9995:", round(z_critico, 4), "\n")
cat("   - Sigma máximo requerido:", round(sigma_nuevo_req, 4), "°C\n")

x_vals_temp <- seq(330, 370, length.out = 500)
df_temp_comp <- data.frame(
    x = rep(x_vals_temp, 2),
    densidad = c(dnorm(x_vals_temp, mu_orig, sigma_orig), dnorm(x_vals_temp, mu_nuevo, sigma_orig)),
    Proceso = rep(c("Centrado (μ=350)", "Descentrado (μ=355)"), each = length(x_vals_temp))
)
ggplot(df_temp_comp, aes(x=x, y=densidad, color=Proceso)) +
    geom_line(linewidth=1.2) +
    geom_vline(xintercept = c(lim_inf, lim_sup), linetype="dashed", color="darkgreen") +
    annotate("rect", xmin=lim_inf, xmax=lim_sup, ymin=0, ymax=Inf, alpha=0.1, fill="green") +
    labs(title="Proceso Centrado vs. Descentrado", x="Temperatura (°C)", y="Densidad") +
    theme_minimal()
```
...
ventas_simuladas <- rnorm(n_meses, mean = mu_ventas, sd = sigma_ventas)
cat("1. Simulación de Ventas (Primeros 6 meses de ", n_meses, "):\n", sep="")
print(round(head(ventas_simuladas), 2))
cat("\n")

prob_mayor_60k <- pnorm(60000, mean = mu_ventas, sd = sigma_ventas, lower.tail = FALSE)

prob_menor_30k <- pnorm(30000, mean = mu_ventas, sd = sigma_ventas)
meses_esperados_menor_30k <- n_meses * prob_menor_30k

q1_ventas <- qnorm(0.25, mean = mu_ventas, sd = sigma_ventas)
q3_ventas <- qnorm(0.75, mean = mu_ventas, sd = sigma_ventas)
iqr_ventas <- q3_ventas - q1_ventas

prob_mayor_45k <- pnorm(45000, mean = mu_ventas, sd = sigma_ventas, lower.tail = FALSE)
porc_meta_cumplida <- prob_mayor_45k * 100

cat("Resultados del Análisis de Ventas:\n")
cat("=================================\n")
cat("2. Probabilidad de ventas > $60,000:", round(prob_mayor_60k, 4), "\n")
cat("3. Meses esperados con ventas < $30,000 (de 24):", round(meses_esperados_menor_30k, 2), "\n")
cat("4. Rango Intercuartílico (Q3 - Q1): $", round(iqr_ventas, 2), 
    " [Q1=", round(q1_ventas,2), ", Q3=", round(q3_ventas,2), "]\n", sep="")
cat("5. Porcentaje esperado de meses cumpliendo meta (> $45,000):", round(porc_meta_cumplida, 2), "%\n")
```
...
vida_simulada <- rnorm(n_sim_bat, mean = mu_bat, sd = sigma_bat)
media_sim_bat <- mean(vida_simulada)
mediana_sim_bat <- median(vida_simulada)

garantia_horas <- qnorm(0.05, mean = mu_bat, sd = sigma_bat)

n_vendidas <- 10000
garantia_fija <- 900
prob_fallo <- pnorm(garantia_fija, mean = mu_bat, sd = sigma_bat)
fallos_esperados <- n_vendidas * prob_fallo


percentil_10 <- qnorm(0.10, mean = mu_bat, sd = sigma_bat)

cat("Análisis del Tiempo de Vida de Baterías:\n")
cat("=========================================\n")
cat("1. Se simularon", n_sim_bat, "baterías.\n")
cat("4. Comparación de Estadísticos:\n")
cat("   - Media Simulada:", round(media_sim_bat, 2), "h (Teórica:", mu_bat, "h)\n")
cat("   - Mediana Simulada:", round(mediana_sim_bat, 2), "h (Teórica:", mu_bat, "h)\n\n")
cat("2. Garantía que cubre al 95% (Percentil 5):", round(garantia_horas, 1), "horas\n")
cat("3. Fallos esperados (de", n_vendidas, ") con garantía de", garantia_fija, "h:", 
    round(fallos_esperados), "(Prob. fallo individual:", round(prob_fallo, 4), ")\n")
cat("5. Percentil 10 de la distribución:", round(percentil_10, 1), "horas\n")

hist(vida_simulada, breaks=30, freq=FALSE, main="Simulación Vida Baterías", xlab="Horas")
curve(dnorm(x, mean=mu_bat, sd=sigma_bat), add=TRUE, col="red", lwd=2)
abline(v=c(garantia_horas, percentil_10), col=c("blue", "green"), lty=2, lwd=2)
legend("topright", legend=c("Densidad Teórica", "Garantía (P5)", "Percentil 10"), 
...
# 1. Simulación
errores_sim <- rnorm(n_sim_err, mean = mu_err, sd = sigma_err)

# 2. Porcentaje entre -5 y +5 (Simulado y Teórico)
porc_entre_5_sim <- mean(errores_sim >= -5 & errores_sim <= 5) * 100
prob_entre_5_teo <- pnorm(5, mu_err, sigma_err) - pnorm(-5, mu_err, sigma_err)
porc_entre_5_teo <- prob_entre_5_teo * 100

# 3. P(|X| > 6) = P(X > 6) + P(X < -6) - Teórico
prob_abs_mayor_6 <- (1 - pnorm(6, mu_err, sigma_err)) + pnorm(-6, mu_err, sigma_err)
# O usando simetría: 2 * pnorm(-6, mu_err, sigma_err)

# 4. Porcentaje preciso |X| <= 3 (Simulado y Teórico)
porc_preciso_sim <- mean(abs(errores_sim) <= 3) * 100
prob_preciso_teo <- pnorm(3, mu_err, sigma_err) - pnorm(-3, mu_err, sigma_err)
porc_preciso_teo <- prob_preciso_teo * 100

# 5. Verificar simetría
media_sim_err <- mean(errores_sim)
mediana_sim_err <- median(errores_sim)
asimetria_sim_err <- moments::skewness(errores_sim) # Usando paquete 'moments'

cat("Análisis de Errores de Medición:\n")
cat("===============================\n")
cat("1. Se simularon", n_sim_err, "errores.\n")
cat("2. Porcentaje entre [-5, 5]: Simulado=", round(porc_entre_5_sim, 2), "%, Teórico=", round(porc_entre_5_teo, 2), "%\n", sep="")
cat("3. Probabilidad Teórica |Error| > 6:", round(prob_abs_mayor_6, 5), "\n")
cat("4. Porcentaje Preciso (|Error|<=3): Simulado=", round(porc_preciso_sim, 2), "%, Teórico=", round(porc_preciso_teo, 2), "%\n", sep="")
cat("5. Verificación de Simetría:\n")
cat("   - Media Simulada:", round(media_sim_err, 4), "(Teórica: 0)\n")
cat("   - Mediana Simulada:", round(mediana_sim_err, 4), "(Teórica: 0)\n")
cat("   - Coeficiente de Asimetría Simulado:", round(asimetria_sim_err, 4), "(Teórico: 0)\n")

# Histograma para verificar visualmente simetría
hist(errores_sim, breaks=30, freq=FALSE, main="Histograma de Errores Simulados", xlab="Error")
curve(dnorm(x, mean=mu_err, sd=sigma_err), add=TRUE, col="red", lwd=2)
...
# 1. Simulación
grupo_A <- rnorm(n_grupo, mean = mu_A, sd = sigma_A)
grupo_B <- rnorm(n_grupo, mean = mu_B, sd = sigma_B)

# 2. Porcentaje Grupo A > 82 (Simulado y Teórico)
porc_A_mayor_82_sim <- mean(grupo_A > 82) * 100
prob_A_mayor_82_teo <- pnorm(82, mean = mu_A, sd = sigma_A, lower.tail = FALSE)
porc_A_mayor_82_teo <- prob_A_mayor_82_teo * 100

# 3. P(X_B < 70) - Teórico
prob_B_menor_70 <- pnorm(70, mean = mu_B, sd = sigma_B)

# 5. Puntuación 10% superior (Percentil 90) - Teórico
p90_A <- qnorm(0.90, mean = mu_A, sd = sigma_A)
p90_B <- qnorm(0.90, mean = mu_B, sd = sigma_B)

cat("Análisis Comparativo de Exámenes:\n")
cat("=================================\n")
cat("1. Se simularon", n_grupo, "estudiantes por grupo.\n")
cat("2. Porcentaje Grupo A > 82: Simulado=", round(porc_A_mayor_82_sim, 2), "%, Teórico=", round(porc_A_mayor_82_teo, 2), "%\n", sep="")
cat("3. Probabilidad Teórica Grupo B < 70:", round(prob_B_menor_70, 4), "\n")
cat("5. Puntuación mínima para el 10% superior:\n")
cat("   - Grupo A (P90):", round(p90_A, 1), "\n")
cat("   - Grupo B (P90):", round(p90_B, 1), "\n")

# 4. Comparación Visual
# Crear data frame combinado
data_examenes <- data.frame(
  puntuacion = c(grupo_A, grupo_B),
  grupo = rep(c("Grupo A (N(75, 10²))", "Grupo B (N(82, 8²))"), each = n_grupo)
)

grafico_comparativo <- ggplot(data_examenes, aes(x = puntuacion, fill = grupo)) +
  geom_density(alpha = 0.6) + # Gráfico de densidad
  # También se podría usar geom_histogram(aes(y=after_stat(density)), alpha=0.6, position="identity", bins=20)
  scale_fill_manual(values = c("Grupo A (N(75, 10²))" = "red", "Grupo B (N(82, 8²))" = "blue")) +
  ...
  labs(
    ...
    subtitle = "Líneas discontinuas indican las medias de cada grupo",
