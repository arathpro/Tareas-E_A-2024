...
```{r}
#| label: solucion-simulaciones-ji
#| code-fold: true
#| warning: false
#| message: false
#| fig-width: 10
...
plots_convergencia <- list()
plot_index <- 1

for (n_df in grados_libertad_lista) {
  
  media_teorica <- n_df
  varianza_teorica <- 2 * n_df
  muestra_completa <- rchisq(n_max, df = n_df)
  
  medias_acumuladas <- numeric(length(tamanos))
  varianzas_acumuladas <- numeric(length(tamanos))
  for (i in 1:length(tamanos)) {
    n_actual <- tamanos[i]
    muestra_parcial <- muestra_completa[1:n_actual]
    medias_acumuladas[i] <- mean(muestra_parcial)
    varianzas_acumuladas[i] <- var(muestra_parcial)
  }
  
  data_conv_media <- data.frame(n = tamanos, Media_Muestral = medias_acumuladas)
  data_conv_var <- data.frame(n = tamanos, Varianza_Muestral = varianzas_acumuladas)
  
  # 1. Gráfico de convergencia de la media
  grafico_media <- ggplot(data_conv_media) +
    geom_line(aes(x = n, y = Media_Muestral), color = "lightblue", linewidth = 1) +
    geom_hline(yintercept = media_teorica, color = "plum", linetype = "dashed", linewidth = 1.2) +
    labs(
      title = paste("Convergencia Media Muestral (n=", n_df, ")"),
      subtitle = paste("Teórica =", media_teorica),
      x = "Tamaño de Muestra", y = "Media Acumulada"
    ) +
    theme_minimal()
  
  # 2. Gráfico de convergencia de la varianza
  grafico_varianza <- ggplot(data_conv_var) +
    geom_line(aes(x = n, y = Varianza_Muestral), color = "lightblue", linewidth = 1) +
    geom_hline(yintercept = varianza_teorica, color = "plum", linetype = "dashed", linewidth = 1.2) +
    labs(
      title = paste("Convergencia Varianza Muestral (n=", n_df, ")"),
      subtitle = paste("Teórica =", varianza_teorica),
      x = "Tamaño de Muestra", y = "Varianza Acumulada"
    ) +
    theme_minimal()
    
  # 3. Histograma vs PDF Teórica (para n_max)
  data_histograma <- data.frame(valores = muestra_completa)
  
  grafico_histograma <- ggplot(data_histograma, aes(x = valores)) +
    geom_histogram(aes(y = after_stat(density)), bins = 50, fill = "darkgreen", alpha = 0.7, color = "white") +
    stat_function(
      fun = dchisq,
      args = list(df = n_df),
      color = "darkgreen", # Amarillo
      linewidth = 1.5
    ) +
    labs(
      title = paste("Histograma vs PDF Teórica (n=", n_df, ")"),
      subtitle = paste("Muestra de tamaño N =", format(n_max, big.mark = ",")),
      x = "Valores Simulados", y = "Densidad"
    ) +
    theme_minimal() +
    xlim(0, quantile(muestra_completa, 0.995)) # Ajustar eje x

  plots_convergencia[[plot_index]] <- grafico_media
  plots_convergencia[[plot_index + 1]] <- grafico_varianza
  plots_convergencia[[plot_index + 2]] <- grafico_histograma
  plot_index <- plot_index + 3
}

grid.arrange(grobs = plots_convergencia, ncol = 3, 
             top = "Análisis de Simulación para Chi-cuadrada")
```
...
grados_lib_base <- 10 # n para Chi2 original
tamanos_muestra_k <- c(5, 15, 30, 100) # k: tamaño de cada muestra
num_medias <- 2000 # Número de medias muestrales a generar para cada k
set.seed(654) # Para reproducibilidad
media_orig_chi <- grados_lib_base
varianza_orig_chi <- 2 * grados_lib_base

lista_medias_chi <- list()
for (k in tamanos_muestra_k) {
  # Generar 'num_medias' medias muestrales, cada una de 'k' observaciones Chi2(10)
  medias <- replicate(num_medias, mean(rchisq(k, df = grados_lib_base)))
  lista_medias_chi[[paste("k =", k)]] <- medias
}

data_tcl_chi <- data.frame()
for (nombre_k in names(lista_medias_chi)) {
  temp_df <- data.frame(media_muestral = lista_medias_chi[[nombre_k]], k_label = nombre_k)
  data_tcl_chi <- rbind(data_tcl_chi, temp_df)
}
data_tcl_chi$k_label <- factor(data_tcl_chi$k_label, levels = paste("k =", tamanos_muestra_k))

tabla_tcl_stats_chi <- data.frame()
for (nombre_k in names(lista_medias_chi)) {
    k_val <- as.numeric(gsub("k = ", "", nombre_k))
    medias_actuales <- lista_medias_chi[[nombre_k]]
    
    media_obs <- mean(medias_actuales)
    var_obs <- var(medias_actuales)
    
    media_teo_barraX <- media_orig_chi
    var_teo_barraX <- varianza_orig_chi / k_val
    
    temp_stats <- data.frame(k = k_val, Media_Obs = media_obs, Media_Teo = media_teo_barraX,
                             Var_Obs = var_obs, Var_Teo = var_teo_barraX)
    tabla_tcl_stats_chi <- rbind(tabla_tcl_stats_chi, temp_stats)
}

cat("Tabla de Estadísticas de las Medias Muestrales (Chi2 vs TCL):\n")
tabla_kable_tcl_chi <- kable(tabla_tcl_stats_chi, digits = 5, 
                             caption = "Estadísticas Observadas vs. Teóricas (TCL)")
tabla_formateada_tcl_chi <- kable_styling(tabla_kable_tcl_chi, bootstrap_options = "striped")
print(tabla_formateada_tcl_chi)

grafico_tcl_chi <- ggplot(data_tcl_chi, aes(x = media_muestral)) +
  geom_histogram(aes(y = after_stat(density)), bins = 30, fill = "orange", alpha = 0.7, color = "white") +
  # Superponer la curva Normal teórica esperada por el TCL: N(n, 2n/k)
  stat_function(
    fun = dnorm,
    args = list(mean = media_orig_chi, sd = sqrt(varianza_orig_chi / as.numeric(gsub("k = ", "", data_tcl_chi$k_label)))),
    color = "blue", # Azul
    linewidth = 1.1,
    linetype = "dashed"
  ) +
  facet_wrap(~k_label, scales = "free") + 
  labs(
    title = paste("Verificación del TCL con Distribución Chi-cuadrada (n=", grados_lib_base, ")"),
    subtitle = "Distribución de las medias muestrales vs. Normal Teórica (línea azul)",
    x = "Media Muestral (k observaciones)",
    y = "Densidad"
  ) +
  theme_minimal()

print(grafico_tcl_chi)
```
...
```{r}
#| label: solucion-suma-normal-cuadrada
#| code-fold: true
#| warning: false
...
n_sim <- 15000
set.seed(321)
matriz_normales <- matrix(rnorm(n_sim * n_sumandos, mean = 0, sd = 1), 
                          nrow = n_sim, ncol = n_sumandos)

matriz_cuadrados <- matriz_normales^2
sumas_cuadrados <- rowSums(matriz_cuadrados)
data_sumas_z2 <- data.frame(valores_suma = sumas_cuadrados)

grafico_sumas_z2 <- ggplot(data_sumas_z2, aes(x = valores_suma)) +
  geom_histogram(aes(y = after_stat(density)), bins = 50, fill = "darkgreen", alpha = 0.7, color = "white") +
  stat_function(
    fun = dchisq,
    args = list(df = n_sumandos),
    color = "black", # Dorado
    linewidth = 1.3
  ) +
  labs(
    title = paste("Verificación: Suma de", n_sumandos, "N(0,1)² vs. Chi2(", n_sumandos, ")"),
    subtitle = "Histograma de sumas simuladas vs. PDF teórica Chi2 (línea dorada)",
    x = "Suma de Z²",
    y = "Densidad"
  ) +
  theme_minimal() +
  xlim(0, quantile(sumas_cuadrados, 0.995)) # Ajustar eje x

print(grafico_sumas_z2)

media_sim_z2 <- mean(sumas_cuadrados)
var_sim_z2 <- var(sumas_cuadrados)
media_teo_z2 <- n_sumandos # Media teórica de Chi2(n) es n
var_teo_z2 <- 2 * n_sumandos # Varianza teórica de Chi2(n) es 2n

cat("\nComparación de Estadísticas (Suma Z² vs. Chi2(8)):\n")
cat("===================================================\n")
cat("Media Simulada (Suma Z²):", round(media_sim_z2, 4), "(Teórica:", media_teo_z2, ")\n")
cat("Varianza Simulada (Suma Z²):", round(var_sim_z2, 4), "(Teórica:", var_teo_z2, ")\n")
```
...
```{r}
#| label: solucion-aproximacion-normal
#| code-fold: true
#| warning: false
#| message: false
#| fig-width: 10
...
n_aprox_vals <- c(10, 30, 50, 100)
n_sim_aprox <- 15000
set.seed(987)
plots_aprox <- list()

for (n_df in n_aprox_vals) {
  muestra_chi2 <- rchisq(n_sim_aprox, df = n_df)
  
  data_aprox <- data.frame(valores = muestra_chi2)
  media_norm_teo <- n_df
  sd_norm_teo <- sqrt(2 * n_df)
  
  grafico_actual <- ggplot(data_aprox, aes(x = valores)) +
    geom_histogram(aes(y = after_stat(density)), bins = 50, fill = "lightblue", alpha = 0.8, color = "white") +
    stat_function(
      fun = dnorm,
      args = list(mean = media_norm_teo, sd = sd_norm_teo),
      color = "black", # Rojo
      linewidth = 1.2,
      linetype = "dashed"
    ) +
    labs(
      title = paste("Aproximación Normal para Chi2(", n_df, ")"),
      subtitle = "Histograma vs. N(", n_df, ",", 2*n_df, ") teórica (línea roja)",
      x = "Valores Chi-cuadrada",
      y = "Densidad"
    ) +
    theme_minimal() +
    xlim(max(0, n_df - 4*sd_norm_teo), n_df + 4*sd_norm_teo) # Ajustar límites
    
  plots_aprox[[paste("n =", n_df)]] <- grafico_actual
}

grid.arrange(grobs = plots_aprox, ncol = 2, 
             top = "Verificación de la Aproximación Normal a Chi-cuadrada")
```
...
muestra_X1 <- rchisq(n_sim_ad, df = n1)
muestra_X2 <- rchisq(n_sim_ad, df = n2)

suma_Y <- muestra_X1 + muestra_X2
data_suma_ji <- data.frame(valores_Y = suma_Y)

grafico_suma_ji <- ggplot(data_suma_ji, aes(x = valores_Y)) +
  geom_histogram(aes(y = after_stat(density)), bins = 50, fill = "orange", alpha = 0.8, color = "white") +
  stat_function(
    fun = dchisq,
    args = list(df = n_suma),
  ...
  labs(
    title = paste("Verificación Aditividad: Chi2(", n1, ") + Chi2(", n2, ") vs. Chi2(", n_suma, ")"),
    subtitle = "Histograma de sumas simuladas vs. PDF teórica Chi2 (línea verde)",
    x = "Suma Simulada (Y = X1 + X2)",
    y = "Densidad"
  ) +
  theme_minimal() +
  xlim(0, quantile(suma_Y, 0.995)) # Ajustar eje x

print(grafico_suma_ji)

media_sim_Y <- mean(suma_Y)
var_sim_Y <- var(suma_Y)
media_teo_Y <- n_suma # Media teórica de Chi2(n1+n2) es n1+n2
var_teo_Y <- 2 * n_suma # Varianza teórica de Chi2(n1+n2) es 2*(n1+n2)

cat("\nComparación de Estadísticas (Suma Y vs. Chi2(15)):\n")
cat("====================================================\n")
cat("Media Simulada (Y):", round(media_sim_Y, 4), "(Teórica:", media_teo_Y, ")\n")
cat("Varianza Simulada (Y):", round(var_sim_Y, 4), "(Teórica:", var_teo_Y, ")\n")
```
