...
```{r}
#| label: solucion-muestras-tstudent
#| code-fold: true
#| warning: false
#| message: false
...
resultados_df <- data.frame(
  ...
  Media_Muestral = double(),
  Varianza_Muestral = double()
)

for (n in tamanos_muestra) {
  muestra_t <- rt(n, df = grados_libertad)
  media_muestral <- mean(muestra_t)
  varianza_muestral <- var(muestra_t)
  ...
  if (n == max(tamanos_muestra)) {
    muestra_grande <- muestra_t
  }
}

cat("Comparación de Estadísticas Muestrales vs. Teóricas (t(5)):\n")
cat("=========================================================\n")
cat("Media Teórica:", media_teorica, "\n")
cat("Varianza Teórica:", round(varianza_teorica, 4), "\n\n")

tabla_resultados <- kable(resultados_df,
                          digits = 4,
                          caption = "Estadísticas Muestrales Calculadas")
tabla_formateada <- kable_styling(tabla_resultados, bootstrap_options = "striped")
print(tabla_formateada)

data_hist <- data.frame(valores = muestra_grande)

grafico_hist <- ggplot(data_hist, aes(x = valores)) +
  geom_histogram(aes(y = after_stat(density)), bins = 30, fill = "darkgreen", alpha = 0.7, color = "white") +
  ...
  labs(
    title = paste("Histograma de Muestra t(5) vs. Densidades Teóricas (n=", max(tamanos_muestra), ")"),
    subtitle = "Azul: t(5) teórica, Rojo discontinuo: N(0,1) teórica",
    x = "Valores Simulados",
    y = "Densidad"
  ) +
  theme_minimal() +
  xlim(quantile(muestra_grande, 0.01), quantile(muestra_grande, 0.99)) # Limitar eje x
...
```{r}
#| label: solucion-relacion-normal-ji
#| code-fold: true
#| warning: false
...
muestra_X <- rnorm(n_sim, mean = 0, sd = 1)
muestra_Y <- rchisq(n_sim, df = n_chi)
muestra_T <- muestra_X / sqrt(muestra_Y / n_chi)

data_hist_T <- data.frame(T_valores = muestra_T)

grafico_T <- ggplot(data_hist_T, aes(x = T_valores)) +
  ...
  labs(
    title = paste("Verificación: T  vs. t(", n_chi, ")"),
    subtitle = paste("X ~ N(0,1), Y ~ Chi2(", n_chi, "). Púrpura: PDF teórica t(", n_chi, ")"),
    x = "Valores Simulados de T",
    y = "Densidad"
  ) +
  theme_minimal() +
  xlim(quantile(muestra_T, 0.005), quantile(muestra_T, 0.995)) # Ajustar límites

print(grafico_T)
media_sim_T <- mean(muestra_T)
var_sim_T <- var(muestra_T)
media_teo_T <- 0
var_teo_T <- n_chi / (n_chi - 2)

cat("\nComparación de Estadísticas (T vs. t(10)):\n")
cat("=========================================\n")
cat("Media Simulada (T):", round(media_sim_T, 4), "(Teórica: ", media_teo_T, ")\n")
cat("Varianza Simulada (T):", round(var_sim_T, 4), "(Teórica: ", round(var_teo_T, 4), ")\n")
```
...
num_muestras <- 1000
mu_teorico <- 0 
grados_lib_t <- tamano_muestra - 1
set.seed(789)
valores_T <- numeric(num_muestras)

for (i in 1:num_muestras) {
  
  muestra_normal <- rnorm(tamano_muestra, mean = mu_teorico, sd = 1)
  media_muestral <- mean(muestra_normal)
  sd_muestral <- sd(muestra_normal)
 
  if (sd_muestral > 1e-10) {
      valores_T[i] <- (media_muestral - mu_teorico) / (sd_muestral / sqrt(tamano_muestra))
  } else {
      valores_T[i] <- NA 
  }
}

valores_T <- na.omit(valores_T) 
data_hist_T_normal <- data.frame(T_calculados = valores_T)

grafico_T_normal <- ggplot(data_hist_T_normal, aes(x = T_calculados)) +
  geom_histogram(aes(y = after_stat(density)), bins = 40, fill = "red", alpha = 0.7, color = "white") +
  stat_function(
    fun = dt,
    args = list(df = grados_lib_t),
    color = "blue", # Gris oscuro
    linewidth = 1.2
  ) +
  labs(
    title = paste("Verificación: T ", grados_lib_t, ")"),
    subtitle = paste("Muestras de N(0,1) de tamaño n=", tamano_muestra, ". Gris: PDF teórica t(", grados_lib_t, ")"),
    x = "Valores Calculados de T",
    y = "Densidad"
  ) +
  theme_minimal() +
   xlim(quantile(valores_T, 0.005, na.rm=TRUE), quantile(valores_T, 0.995, na.rm=TRUE)) # Ajustar límites

print(grafico_T_normal)

media_sim_T_n <- mean(valores_T)
var_sim_T_n <- var(valores_T)
media_teo_T_n <- 0

var_teo_T_n <- grados_lib_t / (grados_lib_t - 2)

cat("\nComparación de Estadísticas (T calculado vs. t(9)):\n")
cat("====================================================\n")
cat("Media Simulada (T):", round(media_sim_T_n, 4), "(Teórica: ", media_teo_T_n, ")\n")
cat("Varianza Simulada (T):", round(var_sim_T_n, 4), "(Teórica: ", round(var_teo_T_n, 4), ")\n")
```
