...
sim_t <- rt(n_sim, df = n_df)
sim_t_squared <- sim_t^2
data_a <- data.frame(valores = sim_t_squared)
grafico_a <- ggplot(data_a, aes(x = valores)) +
  geom_histogram(aes(y = after_stat(density)), bins = 50, fill = "plum", alpha = 0.7, color = "white") +
  stat_function(
    fun = df,
    args = list(df1 = 1, df2 = n_df),
    color = "lightblue",
    linewidth = 1.2
  ) +
  labs(
    title = "Verificación: Si X ~ t(10), entonces X² ~ F(1, 10)",
    subtitle = "Histograma de X² simulado vs. PDF teórica de F(1, 10) (línea roja)",
    x = "Valores Simulados de X²",
    y = "Densidad"
  ) +
  theme_minimal() +
  xlim(0, quantile(sim_t_squared, 0.99)) # Limitar eje x para mejor visualización

print(grafico_a)

media_sim_a <- mean(sim_t_squared)
media_teo_a <- n_df / (n_df - 2)
cat("\nComparación de Medias (Propiedad a):\n")
cat("Media Simulada (X²):", round(media_sim_a, 4), "\n")
cat("Media Teórica F(1, 10):", round(media_teo_a, 4), "\n")
```
...
b <- 15
set.seed(456)
sim_f_ab <- rf(n_sim, df1 = a, df2 = b)

sim_reciprocal <- 1 / sim_f_ab
data_b <- data.frame(valores = sim_reciprocal)

grafico_b <- ggplot(data_b, aes(x = valores)) +
  geom_histogram(aes(y = after_stat(density)), bins = 50, fill = "darkgreen", alpha = 0.7, color = "white") +
  stat_function(
    fun = df,
    args = list(df1 = b, df2 = a), # Parámetros invertidos!
    color = "orange",
    linewidth = 1.2
  ) +
  labs(
    title = paste("Verificación: Si X ~ F(", a, ",", b, "), entonces 1/X ~ F(", b, ",", a, ")"),
    subtitle = "Histograma de 1/X simulado vs. PDF teórica de F(15, 5) (línea naranja)",
    x = "Valores Simulados de 1/X",
    y = "Densidad"
  ) +
  theme_minimal() +
  xlim(0, quantile(sim_reciprocal, 0.99))

print(grafico_b)
if (a > 2) {
  media_sim_b <- mean(sim_reciprocal)
  media_teo_b <- a / (a - 2)
  cat("\nComparación de Medias (Propiedad b):\n")
  cat("Media Simulada (1/X):", round(media_sim_b, 4), "\n")
  cat("Media Teórica F(15, 5):", round(media_teo_b, 4), "\n")
} else {
  cat("\nMedia teórica para F(", b, ",", a, ") no existe pues a <= 2.\n")
}
```
...
sim_chi_a <- rchisq(n_sim, df = a_chi)
sim_chi_b <- rchisq(n_sim, df = b_chi)
sim_f_ratio <- (sim_chi_a / a_chi) / (sim_chi_b / b_chi)

data_c <- data.frame(valores = sim_f_ratio)

grafico_c <- ggplot(data_c, aes(x = valores)) +
  geom_histogram(aes(y = after_stat(density)), bins = 50, fill = "plum", alpha = 0.7, color = "white") +
  stat_function(
    fun = df,
    args = list(df1 = a_chi, df2 = b_chi),
    color = "pink",
    linewidth = 1.2
  ) +
  labs(
    title = paste("Verificación: Si X~Chi2(", a_chi, "), Y~Chi2(", b_chi, "), entonces (X/", a_chi, ")/(Y/", b_chi, ") ~ F(", a_chi, ",", b_chi, ")"),
    subtitle = "Histograma del cociente simulado vs. PDF teórica de F(8, 12) (línea amarilla)",
    x = "Valores Simulados del Cociente",
    y = "Densidad"
  ) +
  theme_minimal() +
  xlim(0, quantile(sim_f_ratio, 0.99))

print(grafico_c)

if (b_chi > 2) {
  media_sim_c <- mean(sim_f_ratio)
  media_teo_c <- b_chi / (b_chi - 2)
  cat("\nComparación de Medias (Propiedad c):\n")
  cat("Media Simulada (Cociente):", round(media_sim_c, 4), "\n")
  cat("Media Teórica F(8, 12):", round(media_teo_c, 4), "\n")
}
```
