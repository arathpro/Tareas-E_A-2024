...
```{r}
#| label: solucion-simulacion-beta
#| code-fold: true
#| warning: false
#| message: false
...
set.seed(123)
muestra_completa <- rbeta(n_max, shape1 = a, shape2 = b)
...
data_convergencia_media <- data.frame(n = tamanos, Media_Muestral = medias_acumuladas)
data_convergencia_var <- data.frame(n = tamanos, Varianza_Muestral = varianzas_acumuladas)

grafico_media <- ggplot(data_convergencia_media) +
  geom_line(aes(x = n, y = Media_Muestral), color = "blue", linewidth = 1) +
  geom_hline(yintercept = media_teorica, color = "lightblue", linetype = "dashed", linewidth = 1.2) +
  labs(
    title = "Convergencia de la Media Muestral",
    subtitle = paste("Beta(", a, ",", b, ") - Media teórica =", round(media_teorica, 4)),
    x = "Tamaño de Muestra (n)",
    y = "Media Muestral Acumulada"
  ) +
  theme_minimal()

grafico_varianza <- ggplot(data_convergencia_var) +
  geom_line(aes(x = n, y = Varianza_Muestral), color = "plum", linewidth = 1) +
  geom_hline(yintercept = varianza_teorica, color = "red", linetype = "dashed", linewidth = 1.2) +
  labs(
    title = "Convergencia de la Varianza Muestral",
    subtitle = paste("Beta(", a, ",", b, ") - Varianza teórica =", round(varianza_teorica, 4)),
    x = "Tamaño de Muestra (n)",
    y = "Varianza Muestral Acumulada"
  ) +
  theme_minimal()

print(grafico_media)
print(grafico_varianza)
...
grafico_histograma <- ggplot(data_histograma, aes(x = valores)) +
  geom_histogram(aes(y = after_stat(density)), bins = 40, fill = "lightblue", alpha = 0.7, color = "white") +
  stat_function(
    fun = dbeta,
  ...
  labs(
    title = paste("Histograma vs PDF Teórica para Beta(", a, ",", b, ")"),
    subtitle = paste("Tamaño de muestra n =", format(n_max, big.mark = ",")),
    x = "Valores Simulados",
    y = "Densidad"
  ) +
  theme_minimal()
...
media_orig <- a / (a + b)
varianza_orig <- (a * b) / ((a + b)^2 * (a + b + 1))

tamanos_muestra_tcl <- c(5, 10, 30, 100) # Tamaños de muestra a probar
num_muestras <- 3000 # Número de veces que repetimos el muestreo para cada n
set.seed(456) # Para reproducibilidad

lista_medias <- list()

for (n in tamanos_muestra_tcl) {
  # Usamos replicate para generar 'num_muestras' medias muestrales
  medias <- replicate(num_muestras, mean(rbeta(n, shape1 = a, shape2 = b)))
  # Guardamos las medias en una lista, nombrando el elemento con n
  lista_medias[[paste("n =", n)]] <- medias
}

data_tcl <- data.frame()
for (nombre_n in names(lista_medias)) {
  temp_df <- data.frame(media_muestral = lista_medias[[nombre_n]], n_label = nombre_n)
  data_tcl <- rbind(data_tcl, temp_df)
}

data_tcl$n_label <- factor(data_tcl$n_label, levels = paste("n =", tamanos_muestra_tcl))

grafico_tcl <- ggplot(data_tcl, aes(x = media_muestral)) +
  geom_histogram(aes(y = after_stat(density)), bins = 30, fill = "plum", alpha = 0.7, color = "white") +
  # Superponer la curva Normal teórica esperada por el TCL
  stat_function(
    fun = dnorm,
    args = list(mean = media_orig, sd = sqrt(varianza_orig / as.numeric(gsub("n = ", "", data_tcl$n_label)))),
    color = "black",
    linewidth = 1.2,
    linetype = "dashed"
  ) +
  facet_wrap(~n_label, scales = "free_y") + # Un panel para cada tamaño de muestra
  labs(
    title = "Verificación del Teorema Central del Límite con Beta(2, 5)",
...
tabla_tcl_stats <- data.frame()
for (nombre_n in names(lista_medias)) {
    n_val <- as.numeric(gsub("n = ", "", nombre_n))
    medias_actuales <- lista_medias[[nombre_n]]
    
    media_obs <- mean(medias_actuales)
    var_obs <- var(medias_actuales)
    
    media_teo_barraX <- media_orig
    var_teo_barraX <- varianza_orig / n_val
    
    temp_stats <- data.frame(n = n_val, Media_Obs = media_obs, Media_Teo = media_teo_barraX,
                             Var_Obs = var_obs, Var_Teo = var_teo_barraX)
    tabla_tcl_stats <- rbind(tabla_tcl_stats, temp_stats)
}

cat("\nTabla de Estadísticas de las Medias Muestrales:\n")
print(kable(tabla_tcl_stats, digits = 5, caption = "Estadísticas Observadas vs. Teóricas (TCL)"))
```
