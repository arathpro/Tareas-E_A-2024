...
```{r}
#| label: funcion-comparativa-simple
#| code-fold: true

comparar_geo_vs_binneg <- function(r, p, n_sim) {
  set.seed(123) # Para reproducibilidad

  data_comparacion <- data.frame(
    valores = c(replicate(n_sim, sum(rgeom(r, prob = p))),  # Método 1
                rnbinom(n_sim, size = r, prob = p)),       # Método 2
    metodo = rep(c(paste("Suma de r=", r, " Geométricas"), 
                   paste("Binomial Negativa (r=", r, ")")),
                 each = n_sim)
  )
  grafico <- ggplot(data_comparacion, aes(x = valores, fill = metodo)) +
    ...
    facet_wrap(~metodo, ncol = 1) +
    labs(
      title = "Verificación: Suma de Geométricas vs. Binomial Negativa",
...
comparar_geo_vs_binneg(r = 5, p = 0.4, n_sim = 10000)
```
...
# b) Cálculo del número esperado (teórico) de fallos
media_teorica_tiros <- r_tiros * (1 - p_tiros) / p_tiros

# c) Simulación de 1000 sesiones
set.seed(456)
sesiones_simuladas <- rnbinom(n_sim_tiros, size = r_tiros, prob = p_tiros)

# Media observada en la simulación
media_simulada_tiros <- mean(sesiones_simuladas)

# Mostrar resultados
cat("Resultados del Ejercicio de Baloncesto\n")
cat("=========================================\n")
cat("b) Número esperado de tiros fallados (Teórico):", round(media_teorica_tiros, 4), "\n")
cat("c) Número esperado de tiros fallados (Simulación):", round(media_simulada_tiros, 4), "\n")
cat("   Diferencia:", abs(media_teorica_tiros - media_simulada_tiros), "\n")

# Visualización de la simulación
ggplot(data.frame(fallos = sesiones_simuladas), aes(x = fallos)) +
  geom_histogram(aes(y = after_stat(density)), bins = 15, fill = "#AF7AC5", alpha = 0.8) +
  geom_vline(xintercept = media_teorica_tiros, color = "red", linetype = "dashed", linewidth = 1.2) +
  labs(
    title = "Simulación de Sesiones de Entrenamiento (1000)",
    subtitle = paste("Línea roja: Media teórica =", round(media_teorica_tiros, 2), "fallos"),
    x = "Número de Tiros Fallados antes de 8 Éxitos",
    y = "Densidad"
  ) +
  theme_minimal()
```
...
```{r}
#| label: solucion-biotecnologia-simple
#| code-fold: true
...
prob_12_fallidas <- dnbinom(x = 12, size = r_pruebas, prob = p_pruebas)
prob_mas_20_fallidas <- 1 - pnbinom(q = 20, size = r_pruebas, prob = p_pruebas)
fallos_esperados <- r_pruebas * (1 - p_pruebas) / p_pruebas

cat("Resultados del Ejercicio de Biotecnología\n")
cat("=================\n")
cat("a) Probabilidad de tener 12 fallos:", round(prob_12_fallidas, 4), "\n")
cat("b) Probabilidad de tener más de 20 fallos:", round(prob_mas_20_fallidas, 4), "\n")
cat("c) Costo esperado en pruebas fallidas: $", format(fallos_esperados * costo_prueba, big.mark = ","), "\n")

data.frame(k = 0:30) %>%
  mutate(probabilidad = dnbinom(k, size = r_pruebas, prob = p_pruebas)) %>%
  ggplot(aes(x = k, y = probabilidad)) +
  geom_col(fill = "plum", alpha = 0.8) +
  geom_vline(xintercept = fallos_esperados, color = "black", linetype = "dashed", linewidth = 1) +
  labs(
    title = "Distribución de Pruebas Fallidas",
    subtitle = "Binomial Negativa",
    x = "Número de Pruebas Fallidas",
    y = "Probabilidad"
  ) +
  annotate("text", x = fallos_esperados + 7, y = 0.04, 
           label = paste("Media =", round(fallos_esperados, 2)), color = "black") +
  theme_minimal()
```
