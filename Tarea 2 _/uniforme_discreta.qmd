...
valores_urna <- 1:8
n <- length(valores_urna)
prob_individual <- 1 / n

# Identificar resultados favorables (números pares)
favorables <- c(2, 4, 6, 8)
num_favorables <- length(favorables)

# Calcular la probabilidad
prob_par <- num_favorables * prob_individual

# Mostrar resultados
cat("Resultados del Ejercicio de la Urna:\n")
cat("=====================================\n")
cat("El espacio muestral es:", valores_urna, "\n")
cat("Los resultados favorables (pares) son:", favorables, "\n")
cat("La probabilidad de obtener un número par es:", prob_par, "\n")

# Crear tabla de probabilidades
urna_data <- data.frame(
  Valor = valores_urna,
  Probabilidad = rep(prob_individual, n),
  Es_Par = ifelse(valores_urna %% 2 == 0, "Sí", "No")
)

# Mostrar tabla con kable
tabla_urna <- kable(urna_data,
      caption = "Distribución de Probabilidad de la Urna",
      align = "ccc")
tabla_formateada <- kable_styling(tabla_urna, bootstrap_options = "striped")
...
```{r}
#| label: solucion-enteros
#| code-fold: true

# Definir el conjunto de valores
valores_juego <- 5:15
n_juego <- length(valores_juego)

# a) Calcular la media
media_juego <- mean(valores_juego)

# b) Calcular la varianza
# Varianza = sum((x_i - media)^2) / n
varianza_juego <- sum((valores_juego - media_juego)^2) / n_juego

cat("Resultados del Ejercicio de Selección de Enteros:\n")
cat("================================================\n")
cat("La media de la distribución es:", media_juego, "\n")
cat("La varianza de la distribución es:", round(varianza_juego, 4), "\n")

# Visualización de la distribución
juego_data <- data.frame(
  Valor = valores_juego,
  Probabilidad = rep(1 / n_juego, n_juego)
)

ggplot(juego_data, aes(x = Valor, y = Probabilidad)) +
  geom_col(fill = "plum", alpha = 0.7, width = 0.8) +
  geom_vline(xintercept = media_juego, color = "black", linetype = "dashed", linewidth = 1) +
  labs(
    title = "Distribución Uniforme Discreta entre 5 y 15",
    x = "Número Seleccionado",
    y = "Probabilidad"
  ) +
  scale_x_continuous(breaks = valores_juego) +
  annotate("text", x = media_juego + 2, y = 0.06, 
           label = paste("Media =", media_juego), color = "blue") +
  theme_minimal()
```
...
# Calcular frecuencias relativas observadas
freq_rel_obs <- prop.table(table(factor(muestra_sim, levels = valores_sim)))

# Crear tabla de comparación
tabla_comparacion_sim <- data.frame(
  Valor = valores_sim,
...
tabla_kable_sim <- kable(tabla_comparacion_sim,
      caption = paste("Simulación de", n_sim, "observaciones"),
      digits = 4)
tabla_formateada_sim <- kable_styling(tabla_kable_sim, bootstrap_options = "striped")
print(tabla_formateada_sim)

# Visualización comparativa
# 1. Preparar datos para el gráfico
datos_grafico_long <- reshape(
  tabla_comparacion_sim,
  varying = c("Prob_Teorica", "Freq_Observada"),
  v.names = "Probabilidad",
  timevar = "Tipo",
  times = c("Teórica", "Observada"),
  direction = "long"
)
# Arreglar nombres y factores
rownames(datos_grafico_long) <- NULL
datos_grafico_long$Tipo <- factor(datos_grafico_long$Tipo)

# 2. Generar el gráfico
ggplot(datos_grafico_long, aes(x = factor(Valor), y = Probabilidad, fill = Tipo)) +
  geom_col(position = "dodge", alpha = 0.8) +
  scale_fill_manual(values = c("Teórica" = "plum", "Observada" = "darkgreen")) +
  labs(
    title = "Comparación de Distribuciones Teórica y Simulada",
    subtitle = paste(n_sim, "observaciones de unif{0,1,2,3,4}"),
    x = "Valor Obtenido",
