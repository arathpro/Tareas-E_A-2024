...
```{r}
#| label: solucion-centro-atencion
#| code-fold: true

lambda_base <- 6

# a) P(X = 4) en un minuto. lambda = 6
prob_a <- dpois(4, lambda = lambda_base)

# b) P(X = 0) en 30 segundos. Hay que ajustar lambda.
# lambda para 30 seg = 6 llamadas/min * 0.5 min = 3
lambda_b <- lambda_base * 0.5
prob_b <- dpois(0, lambda = lambda_b)

# c) Número esperado de llamadas en una hora.
# lambda para 1 hora = 6 llamadas/min * 60 min = 360
lambda_c <- lambda_base * 60
esperado_c <- lambda_c

# d) P(8 <= X <= 12) en dos minutos.
# lambda para 2 min = 6 llamadas/min * 2 min = 12
lambda_d <- lambda_base * 2
prob_d <- ppois(12, lambda = lambda_d) - ppois(7, lambda = lambda_d)

cat("Resultados del Centro de Atención Telefónica solo se tienen que converir a %:\n")
cat("===========================================\n")
cat("a) Probabilidad de recibir exactamente 4 llamadas en un minuto:", round(prob_a, 4), "\n")
cat("b) Probabilidad de no recibir llamadas en 30 segundos:", round(prob_b, 4), "\n")
cat("c) Número esperado de llamadas en una hora:", esperado_c, "\n")
cat("d) Probabilidad de recibir entre 8 y 12 llamadas en dos minutos:", round(prob_d, 4), "\n")
```
:::
...
```{r}
#| label: solucion-linea-prod
#| code-fold: true

tasa_base <- 1.5 / 100

# a) P(X = 8) en 500 productos.
lambda_a <- 500 * tasa_base # 7.5
prob_a <- dpois(8, lambda = lambda_a)

# b) P(X >= 10) en 500 productos. lambda = 7.5
prob_b <- 1 - ppois(9, lambda = lambda_a)

# c) P(X >= 1) = 0.95 => 1 - P(X = 0) = 0.95 => P(X = 0) = 0.05
# P(X = 0) = exp(-lambda) => exp(-lambda) = 0.05 => lambda = -log(0.05)
lambda_c <- -log(0.05)
# productos = lambda / tasa_por_producto
productos_c <- lambda_c / tasa_base

# d) Nueva tasa: 1.2 / 100 = 0.012 defectos/producto
tasa_nueva <- 1.2 / 100
lambda_d <- 500 * tasa_nueva # 6
prob_d <- 1 - ppois(9, lambda = lambda_d)

cat("Resultados de la Línea de Producción solo se tienen que converir a %:\n")
cat("=====================================\n")
cat("a) Probabilidad de tener 8 defectos en 500 productos:", round(prob_a, 4), "\n")
cat("b) Probabilidad de tener 10 o más defectos:", round(prob_b, 4), "\n")
cat("c) Se deben fabricar aprox.", ceiling(productos_c), "productos para asegurar P(X>=1)=0.95\n")
cat("d) Nueva probabilidad de tener 10 o más defectos:", round(prob_d, 4), "\n")
```
:::
...
```{r}
#| label: solucion-hospital
#| code-fold: true

lambda_base <- 2.5

# a) P(X = 3) en una hora. lambda = 2.5
prob_a <- dpois(3, lambda = lambda_base)

# b) P(X = 0) en 2 horas.
lambda_b <- lambda_base * 2 # 5
prob_b <- dpois(0, lambda = lambda_b)

# c) P(X > 8) en 4 horas.
lambda_c <- lambda_base * 4 # 10
prob_c <- 1 - ppois(8, lambda = lambda_c)

# d) Encontrar C tal que P(X > C) < 0.10 => P(X <= C) >= 0.90
# Usamos la función cuantil de Poisson (qpois)
lambda_d <- lambda_c # 10
capacidad_minima <- qpois(0.90, lambda = lambda_d)

cat("Resultados del Servicio de Urgencias solo se tienen que converir a %:\n")
cat("====================================\n")
cat("a) Probabilidad de que lleguen 3 pacientes en una hora:", round(prob_a, 4), "\n")
cat("b) Probabilidad de que no llegue nadie en 2 horas:", round(prob_b, 4), "\n")
cat("c) Probabilidad de superar la capacidad de 8 pacientes:", round(prob_c, 4), "\n")
cat("d) La capacidad mínima debería ser", capacidad_minima, "pacientes para P(saturación)<10%\n")
```
:::
...
```{r}
#| label: solucion-tcl-poisson
#| code-fold: true

lambda_tcl <- 7
tamanos_muestra <- c(5, 15, 30, 100)
num_muestras <- 5000
set.seed(123)
...
for (n in tamanos_muestra) {
  ...
  medias <- replicate(num_muestras, mean(rpois(n, lambda_tcl)))
  temp_df <- data.frame(media_muestral = medias, n_label = paste("n =", n))
  data_tcl <- rbind(data_tcl, temp_df)
}
data_tcl$n_label <- factor(data_tcl$n_label, levels = paste("n =", tamanos_muestra))

tabla_convergencia <- data.frame()
for (n in tamanos_muestra) {
  subset_data <- data_tcl[data_tcl$n_label == paste("n =", n),]
  
  media_obs <- mean(subset_data$media_muestral)
  var_obs <- var(subset_data$media_muestral)
  
  media_teo <- lambda_tcl
  var_teo <- lambda_tcl / n
  
  temp_tabla <- data.frame(n = n, Media_Obs = media_obs, Media_Teo = media_teo,
                           Var_Obs = var_obs, Var_Teo = var_teo)
  tabla_convergencia <- rbind(tabla_convergencia, temp_tabla)
}

cat("Tabla de Convergencia de Media y Varianza:\n")
print(kable(tabla_convergencia, digits = 4))

# Visualización
ggplot(data_tcl, aes(x = media_muestral)) +
  geom_histogram(aes(y = after_stat(density)), bins = 30, alpha = 0.7, fill = "plum") +
  # Superponemos la curva normal teórica
  stat_function(
    fun = dnorm,
    args = list(mean = lambda_tcl, sd = sqrt(lambda_tcl / as.numeric(gsub("n = ", "", data_tcl$n_label)))),
    color = "darkred", linewidth = 1, linetype = "dashed"
  ) +
  facet_wrap(~n_label, scales = "free") +
  labs(
    title = "Verificación del TCL con Distribución Poisson (λ=7)",
    subtitle = "Línea roja: Densidad Normal Teórica",
    x = "Media Muestral",
    y = "Densidad"
  ) +
  theme_minimal()
...
# a) Generar muestras y sumarlas
X1 <- rpois(n_sim, lambda1)
X2 <- rpois(n_sim, lambda2)
suma_X1_X2 <- X1 + X2

# Generar muestra de la suma directa
suma_directa <- rpois(n_sim, lambda_suma)

# b) Comparar estadísticas
stats_df <- data.frame(
  Metodo = c("Suma X1+X2", "Directa Poisson(6.3)", "Teórico"),
  Media = c(mean(suma_X1_X2), mean(suma_directa), lambda_suma),
  Varianza = c(var(suma_X1_X2), var(suma_directa), lambda_suma)
)
cat("Comparación de Estadísticas:\n")
print(kable(stats_df, digits = 4))

# Visualización
data_vis_suma <- data.frame(
  valores = c(suma_X1_X2, suma_directa),
  metodo = rep(c("Suma Simulada X1+X2", "Directa Poisson(6.3)"), each = n_sim)
)
ggplot(data_vis_suma, aes(x = valores, fill = metodo)) +
  geom_histogram(alpha = 0.6, binwidth = 1, position = "dodge") +
  labs(title = "Verificación de la Propiedad Aditiva",
       subtitle = "λ1=3.5, λ2=2.8, λ_suma=6.3",
       x = "Número de Eventos", y = "Frecuencia") +
  theme_minimal()

# c) Extensión a 4 variables
lambdas_multi <- c(1.2, 2.0, 0.8, 1.5)
lambda_total_multi <- sum(lambdas_multi)
suma_multi <- rpois(n_sim, lambdas_multi[1]) + rpois(n_sim, lambdas_multi[2]) + 
              rpois(n_sim, lambdas_multi[3]) + rpois(n_sim, lambdas_multi[4])

cat("\nExtensión a 4 variables: Media de la suma simulada:", round(mean(suma_multi), 4),
    "(Teórica:", lambda_total_multi, ")\n")
```
...
```{r}
#| label: solucion-aplicacion-web
#| code-fold: true

lambda_web <- 15
capacidad_actual <- 25

# a) Probabilidad de degradación P(X > 25)
prob_degradacion_a <- 1 - ppois(capacidad_actual, lambda = lambda_web)

# b) Nueva capacidad = 50. P(X > 50)
capacidad_nueva <- 50
prob_degradacion_b <- 1 - ppois(capacidad_nueva, lambda = lambda_web)

# c) Capacidad óptima C para que P(X > C) < 0.01 => P(X <= C) >= 0.99
capacidad_optima <- qpois(0.99, lambda = lambda_web)

# d) Simulación de 60 minutos
set.seed(789)
minutos_simulados <- 60
trafico_hora <- rpois(minutos_simulados, lambda = lambda_web)
minutos_con_degradacion <- sum(trafico_hora > capacidad_actual)
porcentaje_degradacion <- (minutos_con_degradacion / minutos_simulados) * 100

cat("Análisis del Servidor Web:\n")
cat("==========================\n")
cat("a) Probabilidad de degradación (cap > 25):", round(prob_degradacion_a, 4), "\n")
cat("b) Probabilidad de degradación con nueva capacidad (cap > 50):", round(prob_degradacion_b, 6), "\n")
cat("c) Capacidad óptima para P(degradación)<1%:", capacidad_optima, "solicitudes\n")
cat("d) Simulación de 1 hora: Se observó degradación en", minutos_con_degradacion, "minutos (",
    round(porcentaje_degradacion, 2), "% del tiempo)\n")
```
...
# a) P(stockout) si inventario = 12. P(X > 12)
inventario_a <- 12
prob_stockout_a <- 1 - ppois(inventario_a, lambda = lambda_ventas)

# b) Inventario I para que P(X > I) < 0.05 => P(X <= I) >= 0.95
inventario_b <- qpois(0.95, lambda = lambda_ventas)

# d) Determinar el nivel óptimo de inventario
# Función para calcular el costo total esperado para un nivel de inventario I
calcular_costo_total <- function(I, lambda) {
  # Rango de posibles ventas (hasta un punto donde la probabilidad es casi cero)
  x_range <- 0:(qpois(0.9999, lambda))
  prob_x <- dpois(x_range, lambda)
  
  # Costo por sobrantes: E[2 * max(0, I - X)]
  costo_sobrante_esperado <- sum(2 * pmax(0, I - x_range) * prob_x)
  
  # Costo por faltantes: E[15 * max(0, X - I)]
  costo_faltante_esperado <- sum(15 * pmax(0, x_range - I) * prob_x)
  
  # Costo de adquisición
  costo_adquisicion_total <- I * costo_adquisicion
  
  return(costo_adquisicion_total + costo_sobrante_esperado + costo_faltante_esperado)
}

# c) Calcular el costo para un inventario de 12
costo_total_c <- calcular_costo_total(12, lambda_ventas)

# d) Encontrar el inventario óptimo probando un rango de niveles
niveles_inventario <- 5:20
costos_totales <- sapply(niveles_inventario, calcular_costo_total, lambda = lambda_ventas)
inventario_optimo <- niveles_inventario[which.min(costos_totales)]

cat("Análisis de Gestión de Inventarios:\n")
cat("==================================\n")
cat("a) Probabilidad de quedarse sin existencias con 12 unidades:", round(prob_stockout_a, 4), "\n")
cat("b) Inventario necesario para P(agotarse)<5%:", inventario_b, "unidades\n")
cat("c) Costo total esperado con un inventario de 12 unidades: $", round(costo_total_c, 2), "\n")
cat("d) El nivel de inventario óptimo que minimiza el costo es:", inventario_optimo, "unidades\n")

# Visualización para d)
data_costos <- data.frame(Inventario = niveles_inventario, Costo_Total = costos_totales)
ggplot(data_costos, aes(x = Inventario, y = Costo_Total)) +
  geom_line(color = "blue", linewidth = 1) +
  geom_point(color = "blue", size = 3) +
  geom_point(data=data.frame(x=inventario_optimo, y=min(costos_totales)), 
             aes(x=x, y=y), color="red", size=5, shape=18) +
  labs(title = "Optimización de Costo de Inventario",
       x = "Nivel de Inventario Inicial",
       y = "Costo Total Esperado ($)") +
  scale_x_continuous(breaks=niveles_inventario) +
  annotate("text", x = inventario_optimo, y = min(costos_totales) + 2, 
           label=paste("Óptimo:", inventario_optimo, "unidades"), color="red") +
  theme_minimal()
```
