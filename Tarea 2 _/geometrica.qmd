...
```{r}
#| label: solucion-geom-moneda-no-pipe
#| code-fold: true

p_cara <- 0.5

# a) P(X = 4) -> 4 fracasos antes del 5to lanzamiento exitoso
prob_5_lanzamientos <- dgeom(4, p_cara)

# b) P(X >= 3) -> más de 3 lanzamientos es P(X > 2)
prob_mas_3_lanzamientos <- 1 - pgeom(2, p_cara)

# c) Media y Varianza
media_lanzamientos <- (1 - p_cara) / p_cara
varianza_lanzamientos <- (1 - p_cara) / (p_cara^2)

cat("Resultados del Ejercicio de Lanzamiento de Moneda:\n")
cat("===================================================\n")
cat("a) Probabilidad de necesitar exactamente 5 lanzamientos (X=4):", round(prob_5_lanzamientos, 4), "\n")
cat("b) Probabilidad de necesitar más de 3 lanzamientos (X>=3):", round(prob_mas_3_lanzamientos, 4), "\n")
cat("c) Media de fracasos esperados:", media_lanzamientos, "\n")
cat("   Varianza:", varianza_lanzamientos, "\n")

# Visualización
moneda_data_vis <- data.frame(k = 0:10)
moneda_data_vis$probabilidad <- dgeom(moneda_data_vis$k, p_cara)
ggplot(moneda_data_vis, aes(x = k, y = probabilidad)) +
  geom_col(fill = "plum", alpha = 0.8) +
  geom_vline(xintercept = media_lanzamientos, color = "darkred", linetype = "dashed", linewidth = 1) +
  labs(
    title = "Distribución del número de sellos antes de la primera cara",
    subtitle = "Geométrica (p=0.5)",
    x = "Número de Fracasos (Sellos)",
    y = "Probabilidad"
  ) +
  annotate("text", x = media_lanzamientos + 2.5, y = 0.2, 
           label = paste("Media =", round(media_lanzamientos, 2)), color = "darkred") +
  theme_minimal()
```
...
```{r}
#| label: solucion-geom-basket-no-pipe
#| code-fold: true
p_anotar <- 0.7

# a) P(X = 2) -> 2 fracasos antes del 3er intento exitoso
prob_tercer_intento <- dgeom(2, p_anotar)

# b) Intentos fallados esperados (Media)
media_intentos <- (1 - p_anotar) / p_anotar

# c) P(1 <= X <= 4) = P(X <= 4) - P(X <= 0)
prob_2_a_5 <- pgeom(4, p_anotar) - pgeom(0, p_anotar)

cat("Resultados del Ejercicio de Baloncesto:\n")
cat("=========================================\n")
cat("a) Probabilidad de anotar por primera vez en el 3er intento (X=2):", round(prob_tercer_intento, 4), "\n")
cat("b) Número esperado de intentos fallados:", round(media_intentos, 4), "\n")
cat("c) Probabilidad de necesitar entre 2 y 5 intentos (1<=X<=4):", round(prob_2_a_5, 4), "\n")

basket_data_vis <- data.frame(k = 0:8)
basket_data_vis$probabilidad <- dgeom(basket_data_vis$k, p_anotar)

ggplot(basket_data_vis, aes(x = k, y = probabilidad)) +
  geom_col(fill = "plum", alpha = 0.8) +
  geom_vline(xintercept = media_intentos, color = "darkred", linetype = "dashed", linewidth = 1) +
  annotate("rect", xmin = 1, xmax = 4, ymin = 0, ymax = Inf, alpha = 0.2, fill = "blue") +
  labs(
    title = "Distribución de tiros fallados antes del primer acierto",
    subtitle = "Geométrica (p=0.7)",
    x = "Número de Tiros Fallados",
    y = "Probabilidad"
  ) +
  annotate("text", x = media_intentos + 2, y = 0.4, 
           label = paste("Media =", round(media_intentos, 2)), color = "darkblue") +
  theme_minimal()
```
...
```{r}
#| label: solucion-geom-maquina-no-pipe
#| code-fold: true

p_defecto <- 0.02

# a) Número esperado de inspecciones buenas (Media)
media_maquina <- (1 - p_defecto) / p_defecto

# b) P(X >= 100) = 1 - P(X <= 99)
prob_despues_100 <- 1 - pgeom(99, p_defecto)

# c) Demostración de falta de memoria: P(X >= 50 + 100 | X >= 50) = P(X >= 100)
s <- 50
t <- 100
prob_s_mas_t <- 1 - pgeom(s + t - 1, p_defecto)
prob_s <- 1 - pgeom(s - 1, p_defecto)
prob_condicional_memoria <- prob_s_mas_t / prob_s
prob_t_memoria <- 1 - pgeom(t - 1, p_defecto)

cat("Resultados del Ejercicio de la Máquina:\n")
cat("=========================================\n")
cat("a) Número esperado de piezas buenas antes de la 1ra defectuosa:", media_maquina, "\n")
cat("b) Probabilidad de que la 1ra defectuosa sea después de la pieza 100 (X>=100):", round(prob_despues_100, 4), "\n\n")
cat("c) Demostración de Falta de Memoria (s=50, t=100):\n")
cat("   P(X >= 150 | X >= 50) =", round(prob_condicional_memoria, 4), "\n")
cat("   P(X >= 100) =", round(prob_t_memoria, 4), "\n")
cat("   ¿Son iguales?:", abs(prob_condicional_memoria - prob_t_memoria) < 1e-9, "\n")

maquina_data_vis <- data.frame(k = 0:150)
maquina_data_vis$probabilidad <- dgeom(maquina_data_vis$k, p_defecto)
ggplot(maquina_data_vis, aes(x = k, y = probabilidad)) +
  geom_col(fill = "plum", alpha = 0.8) +
  geom_vline(xintercept = media_maquina, color = "darkred", linetype = "dashed", linewidth = 1) +
  labs(
    title = "Distribución de piezas buenas antes de la primera defectuosa",
    subtitle = "Geométrica (p=0.02)",
    x = "Número de Piezas Buenas",
    y = "Probabilidad"
  ) +
  annotate("text", x = media_maquina + 25, y = 0.015, 
           label = paste("Media =", round(media_maquina, 2)), color = "darkred") +
  theme_minimal()
```
