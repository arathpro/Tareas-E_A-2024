...
# Crear la tabla de probabilidades primero
bernoulli_maquina <- data.frame(
  Resultado = c("Pieza no defectuosa (0)", "Pieza defectuosa (1)"),
  Valor = c(0, 1),
  Probabilidad = c(1 - p_defecto, p_defecto)
)

kable(bernoulli_maquina, 
      caption = "Distribución Bernoulli - Máquina que produce piezas defectuosas (p = 0.02)",
      align = "ccc") %>%
  kable_styling(bootstrap_options = "striped")

# Calcular y mostrar las respuestas a las preguntas
prob_no_defecto <- 1 - p_defecto
media_defecto <- p_defecto
varianza_defecto <- p_defecto * (1 - p_defecto)
prob_2_defectos <- dbinom(2, 10, p_defecto)

cat("\nRespuestas a las preguntas:\n")
cat("a) Probabilidad de una pieza no defectuosa (1-p):", prob_no_defecto, "\n")
cat("b) Media (p):", media_defecto, "\n")
cat("   Varianza (p(1-p)):", varianza_defecto, "\n")
cat("c) Probabilidad de encontrar exactamente 2 piezas defectuosas en 10:", round(prob_2_defectos, 4), "\n")

```


```{r}
#| label: grafico-binomial-maquina
#| code-fold: true
#| fig-cap: "Distribución Binomial para el número de piezas defectuosas"
n_piezas <- 10
p_defecto <- 0.02
binomial_data <- data.frame(
  k = 0:n_piezas,
  Probabilidad = dbinom(0:n_piezas, n_piezas, p_defecto)
)


ggplot(binomial_data, aes(x = k, y = Probabilidad)) +
  geom_col(fill = "firebrick", alpha = 0.8, width = 0.7) +
  geom_text(aes(label = round(Probabilidad, 4)), vjust = -0.5, size = 3.5) +
  scale_x_continuous(breaks = 0:n_piezas) +
  labs(
    title = "Probabilidad de Encontrar Piezas Defectuosas",
    subtitle = "Distribución Binomial(n=10, p=0.02)",
    x = "Número de Piezas Defectuosas (k)",
    y = "Probabilidad"
  ) +
  theme_minimal() +
  theme(
...
```{r}
#| label: bernoulli-examen

p_acierto <- 0.25

examen_bernoulli <- data.frame(
  Resultado = c("Incorrecto (0)", "Correcto (1)"),
  Probabilidad = c(1 - p_acierto, p_acierto)
)

kable(examen_bernoulli,
...
cat("Desviación estándar:", round(desv_std_examen, 4), "\n")
```
...
```{r}
#| label: binomial-monedas-ejercicio
# Parámetros
n_lanzamientos <- 15
p_cara <- 0.5
...
# Alternativa: sum(dbinom(12:15, n_lanzamientos, p_cara))

# c) Media y Desviación Estándar
media_caras <- n_lanzamientos * p_cara
varianza_caras <- n_lanzamientos * p_cara * (1 - p_cara)
desv_std_caras <- sqrt(varianza_caras)

# Mostrar resultados
cat("a) Probabilidad de obtener exactamente 8 caras:", round(prob_8_caras, 4), "\n")
cat("b) Probabilidad de obtener al menos 12 caras:", round(prob_min_12_caras, 4), "\n")
cat("c) Media (np):", media_caras, "\n")
cat("   Desviación estándar (sqrt(np(1-p))):", round(desv_std_caras, 4), "\n")

# Visualización
monedas_data <- data.frame(
  k = 0:n_lanzamientos,
  probabilidad = dbinom(0:n_lanzamientos, n_lanzamientos, p_cara)
)

ggplot(monedas_data, aes(x = k, y = probabilidad)) +
  geom_col(fill = "plum", alpha = 0.8) +
  geom_vline(xintercept = media_caras, color = "red", linetype = "dashed", size = 1) +
  annotate("text", x = media_caras + 2.5, y = 0.15,
           label = paste("Media =", media_caras), color = "red") +
  scale_x_continuous(breaks = 0:n_lanzamientos) +
  labs(
    title = "Distribución Binomial - 15 Lanzamientos de Moneda",
    x = "Número de Caras", y = "Probabilidad"
  ) +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5))
```
...
prob_15_a_19 <- pbinom(19, n_pacientes, p_cura) - pbinom(14, n_pacientes, p_cura)

# c) Número esperado de curas (Media)
media_curas <- n_pacientes * p_cura

# Mostrar resultados
cat("a) Probabilidad de que se curen 18 pacientes:", round(prob_18_curas, 4), "\n")
cat("b) Probabilidad de que se curen entre 15 y 19 pacientes:", round(prob_15_a_19, 4), "\n")
cat("c) Número esperado de pacientes curados:", media_curas, "\n")

# Visualización
medicamento_data <- data.frame(
  k = 0:n_pacientes,
  probabilidad = dbinom(0:n_pacientes, n_pacientes, p_cura)
)

ggplot(medicamento_data, aes(x = k, y = probabilidad)) +
  geom_col(fill = "aquamarine", alpha = 0.8) +
  geom_vline(xintercept = media_curas, color = "red", linetype = "dashed", size = 1) +
  annotate("rect", xmin = 15, xmax = 19, ymin = 0, ymax = Inf, alpha = 0.2, fill = "orange") +
  annotate("text", x = media_curas - 2, y = 0.2, label = paste("E[X] =", media_curas), color = "red") +
  labs(
    title = "Distribución del Número de Pacientes Curados",
    subtitle = "Binomial(n=20, p=0.85)",
    x = "Número de Pacientes Curados", y = "Probabilidad"
  ) +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5),
        plot.subtitle = element_text(hjust = 0.5))
...
```{r}
#| label: simulacion-suma-binomial
set.seed(123)
n <- 10
m <- 20
p <- 0.3
...
z_directo <- rbinom(n_sim, n + m, p)

x_sim <- rbinom(n_sim, n, p)
y_sim <- rbinom(n_sim, m, p)
z_suma <- x_sim + y_sim

media_teorica <- (n + m) * p
media_obs_directo <- mean(z_directo)
media_obs_suma <- mean(z_suma)

cat("Media Teórica ( (n+m)p ):", media_teorica, "\n")
cat("Media Observada (Método Directo):", round(media_obs_directo, 4), "\n")
cat("Media Observada (Método de Suma):", round(media_obs_suma, 4), "\n")
...
ggplot(comparacion_df, aes(x = Valor, fill = Metodo)) +
  geom_histogram(alpha = 0.6, position = "identity", binwidth = 1, center = 0) +
  scale_fill_manual(values = c("Directo" = "blue", "Suma_XY" = "red")) +
  labs(
    title = "Equivalencia: Bin(n+m, p) vs Bin(n,p) + Bin(m,p)",
    subtitle = paste("n=", n, ", m=", m, ", p=", p),
    x = "Valor de Z = X+Y", y = "Frecuencia"
  ) +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5),
        plot.subtitle = element_text(hjust = 0.5),
